...
function canvasFromImageNormalize_(img, scale, maxDim, boostContrast, rotateDeg){
  ...
  let rw=iw, rh=ih;
  if(maxSide>maxDim){ const sc=maxDim/maxSide; rw=Math.round(iw*sc); rh=Math.round(ih*sc); }
  rw=Math.max(8,Math.round(rw*scale)); rh=Math.max(8,Math.round(rh*scale));
  const rot=(rotateDeg||0)%360, rad=rot*Math.PI/180;
  const sw=(rot===90||rot===270)?rh:rw, sh=(rot===90||rot===270)?rw:rh;

  const c=document.createElement('canvas'); c.width=sw; c.height=sh;
  const ctx=c.getContext('2d',{alpha:false,desynchronized:true});
  ctx.save(); ctx.translate(sw/2,sh/2); ctx.rotate(rad); ctx.drawImage(img,-rw/2,-rh/2,rw,rh); ctx.restore();
...
on(document,'submit','form.qcForm', async (e,form)=>{
  ...
  qsa('.control-item', form).forEach(ci=>{
    ...
    if(isKO){
      ...
      const hasComment = (qs('textarea', box)?.value||'').trim().length>0;
  ...
  try{
    ...
    alert('Échec réseau: '+(e?.message||e));
  }finally{
    hideLoader();
  }
});
